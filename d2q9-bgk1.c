...
double rebound_collision_av(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles);
...
double timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles)
{
 // accelerate_propagate(params, cells, tmp_cells, obstacles);
  accelerate_flow(params, cells, obstacles);
  //propagate(params, cells, tmp_cells);
  return rebound_collision_av(params, cells, tmp_cells, obstacles);
}

/*      Combining accelerate_flow and propagate functions    */
//int accelerate_propagate(const t_param params, t_speed* cells, t_speed* tmp_cells,int* obstacles)
//{
 ...
 // {
   ...
   // { 
     ...
     // if(ii==0)
     // {
       //  unsigned int zz = params.ny - 2;
       //  unsigned int index = zz * params.nx + jj;
        // if (!obstacles[index]
     ...
     // }
      ...
      /*       propagate function        */
     // unsigned int index1 = ii * params.nx + jj;
      /* determine indices of axis-direction neighbours
      ** respecting periodic boundary conditions (wrap around) */
     // unsigned int y_n = (ii + 1) & (params.ny-1);
     // unsigned int x_e = (jj + 1) & (params.nx-1);
     // unsigned int y_s = (ii == 0) ? (ii + params.ny - 1) : (ii - 1);
     // unsigned int x_w = (jj == 0) ? (jj + params.nx - 1) : (jj - 1);
      /* propagate densities to neighbouring cells, following
      ** appropriate directions of travel and writing into
      ** scratch space grid */
     // tmp_cells[index1].speeds[0]  = cells[index1].speeds[0]; /* central cell, no movement */
     // tmp_cells[ii * params.nx + x_e].speeds[1] = cells[index1].speeds[1]; /* east */
     // tmp_cells[y_n * params.nx + jj].speeds[2]  = cells[index1].speeds[2]; /* north */
     // tmp_cells[ii * params.nx + x_w].speeds[3] = cells[index1].speeds[3]; /* west */
     // tmp_cells[y_s * params.nx + jj].speeds[4]  = cells[index1].speeds[4]; /* south */
     // tmp_cells[y_n * params.nx + x_e].speeds[5] = cells[index1].speeds[5]; /* north-east */
     // tmp_cells[y_n * params.nx + x_w].speeds[6] = cells[index1].speeds[6]; /* north-west */
     // tmp_cells[y_s * params.nx + x_w].speeds[7] = cells[index1].speeds[7]; /* south-west */
     // tmp_cells[y_s * params.nx + x_e].speeds[8] = cells[index1].speeds[8]; /* south-east */
   // }
 // }
 ...
 *   *             ** scratch space grid */
     ...
     // tmp_cells[ii * params.nx + jj].speeds[5] = cells[y_s * params.nx + x_w].speeds[5]; /* north-east */
      // tmp_cells[ii * params.nx + jj].speeds[6] = cells[y_s * params.nx + x_e].speeds[6]; /* north-west */
     // tmp_cells[ii * params.nx + jj].speeds[7] = cells[y_n * params.nx + x_e].speeds[7]; /* south-west */
     // cells[ii * params.nx + jj].speeds[8] = tmp_cells[y_n * params.nx + x_w].speeds[8]; /* south-ea*/ 
 ...
 *  *         ** mirroring, and writing into main grid */
        ...
        /*      av_celocity fucntion     */                         
       /* u0 = tmp_cells[index].speeds[1]
           + tmp_cells[index].speeds[5]
           + tmp_cells[index].speeds[8]
           - (tmp_cells[index].speeds[3]
           + tmp_cells[index].speeds[6]
        ...
        u_x = u0 / local_density;*/
        /* compute y velocity component */
       /* u1 = tmp_cells[index].speeds[2]
           + tmp_cells[index].speeds[5]
           + tmp_cells[index].speeds[6]
           - (tmp_cells[index].speeds[4]
           + tmp_cells[index].speeds[7]
  ...
  for (unsigned int ii=0; ii<params.ny;ii++  )
  {
    for (unsigned  jj=0;jj <params.nx;jj++  )
    {
      ...
      if (!obstacles[index])
      {
        ...
        double u_x = (cells[index].speeds[1]
        ...
        /* compute y velocity component */
        double u_y = (cells[index].speeds[2]
                    + cells[index].speeds[5]
                    + cells[index].speeds[6]
...
double calc_reynolds(const t_param params, t_speed* cells, int* obstacles)
{
  ...
  return av_velocity_serial(params, cells, obstacles) * params.reynolds_dim / viscosity;
}
